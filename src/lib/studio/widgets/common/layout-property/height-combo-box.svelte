<script lang="ts">
    ...
    export type HeightComboBoxItemChangeValue = "select-fixed-height" | "hug-contents" | "fill-container" | "select-min-height" | "add-min-height" | "select-max-height" | "add-max-height" | "delete-min-max";
...
<ComboBox class={className} name='H' {icon} {value} {comboBoxItems} {min} {max} onChange={handleValueChange} onDragStart={onDragStart} onDragEnd={onDragEnd} />
