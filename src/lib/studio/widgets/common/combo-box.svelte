...
{#if showCustomCursor}
	<div
		class="pointer-events-none fixed z-[9999] cursor-ew-resize"
	...
	>
		...
		<svg width="64" height="24" viewBox="0 0 64 24" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			<path
				filter="url(#shadow)"
				d="M44 12l-6 6v-4h-8v-4h8v-4l6 6z"
				fill="black"
				stroke="white"
				stroke-width="2"
			/>
...
<div
	class={cn('relative flex h-6 min-w-0 items-center rounded-sm bg-gray-100 text-xs', className)}
...
>
	...
	>
		...
		{:else}
			<div class="px-1 text-center text-gray-600">
	...
	<input
		type="text"
		class="h-full min-w-0 flex-1 text-ellipsis text-gray-800 outline-none"
		{placeholder}
		onfocus={(event) => {
	...
	{#if !isHovering && showFill}
		<!-- Fill 표시 -->
		<div
			class="pointer-events-none absolute top-0 right-0 left-6 flex h-full items-center justify-end pr-2"
		>
	...
	<Popover.Root bind:open={isPopoverOpen}>
		<Popover.Trigger class="w-6">
			<div class={cn('flex w-full items-center rounded-sm text-xs', className)}>
				<div class="w-6 flex-shrink-0 text-gray-800">
		...
		<Popover.Content class="w-full bg-gray-900 px-2 py-2 text-white" side="bottom" align="end">
			<div class="flex flex-col text-xs">
				{#each comboBoxItems as item}
					{#if item.type === 'item'}
						<button
							type="button"
							class="flex cursor-pointer items-center gap-2 px-2 py-1 text-left hover:bg-gray-500"
							onclick={() => {
